...
Appel d'API sur un Spark Core faisant fonctionner le programme
  ...
  Affiche la valeur du compteur (nombre d'activation du senseur PIR). 
  Affiche l'état actuel du senseur PIR (actuellement).
  Demande un reset du compteur et force la relecture du senseur sur
...
Ou acheter un senseur PIR

   http://shop.mchobby.be/category.php?id_category=61
...
def main():
	...
	if( value[0] == False ):
		...
		actif = core.value_of( 'actif' ) # retourne le tuple (connected, actif)
		print( 'Etat de la variable "actif": %i ' % actif[1] )
		print( 'Le senseur PIR est %s' % ('ACTIF' if actif[1] == 1 else 'inactif' ) )
