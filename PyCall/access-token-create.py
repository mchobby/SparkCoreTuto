...
# -*- coding: utf-8 -*-
#
"""access-token-create.py

Appel d'API sur Spark Cloud pour creer/ajouter un nouvel access_token
liés à votre compte Spark Cloud.
...
def main():
	...
	client_id = 'xytest' # Client ID identifying the access_token
		
	result = api.api_create_access_token( user, passwd, client_id )
	access_token_data = result[1]
	#print( access_token_data )
	
	print( 'created access token: %s' % access_token_data['access_token'] )
	print( 'token type: %s' % access_token_data['token_type'] ) # "bearer" signifie "titulaire"
	print( 'Expires in: %s' % access_token_data['expires_in'] )
